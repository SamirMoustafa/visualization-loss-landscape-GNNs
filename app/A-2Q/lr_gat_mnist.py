...
from a2q.modules import FeatureQuantizationV2, LinearQuantizationV2, WeightQuantizationV2
...
def paras_group(model):
    ...
    params_id = (
            ...
            + list(map(id, quant_paras_scale_gat_attn))
            + list(map(id, quant_paras_scale_gat_fea))
            + list(map(id, quant_paras_scale_gat_edge))
    )
...
def parameter_statistics(model, dataset, hidden_units):
    ...
    for name, par in model.named_parameters():
        ...
        elif ("bit" in name) and ("fea" in name):
            if "conv1" in name:
                a_scale = 0
            else:
...
class qGATConv(MessagePassing):
    def __init__(
        ...
        self.q_gat_attn = WeightQuantizationV2(
            1,
            heads,
            bit=bit,
        )
        self.q_gat_fea = FeatureQuantizationV2(
            input_dim=num_nodes, bit=bit, uniform=uniform, gama_init=0.01, gama_std=0.01, init=init
        )
        self.q_gat_edge = FeatureQuantizationV2(
            input_dim=num_nodes, bit=bit, uniform=uniform, gama_init=0.1, gama_std=0.01, init=init
        )
...
class GAT(torch.nn.Module):
    def __init__(
        ...
        if is_q:
            self.embeding = LinearQuantizationV2(
            ...
            self.embeding = nn.Linear(
                dataset[0].x.size()[1] + dataset[0].pos.size()[1], hidden_units * heads, bias=True
            )
    ...
    for k in range(2):
        ...
        for i in range(args.max_cycle):
            ...
            optimizer = torch.optim.Adam(
                [{"params": weight_paras}, {"params": other_paras}], lr=args.lr, weight_decay=args.weight_decay
            )
