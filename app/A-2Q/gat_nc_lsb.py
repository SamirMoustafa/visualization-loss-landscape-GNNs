...
from a2q.modules import FeatureQuantizationV1, LinearQuantizationV1, WeightQuantizationV1
...
def paras_group(model):
    ...
    params_id = (
            ...
            + list(map(id, quant_paras_bit_gat_fea))
            + list(map(id, quant_paras_bit_gat))
            + list(map(id, quant_paras_scale_gat))
            + list(map(id, quant_paras_scale_gat_fea))
    )
...
def parameter_statistics(model, dataset, hidden_units, heads):
    w_Byte = torch.tensor(0)
    a_Byte = torch.tensor(0)
    for name, par in model.named_parameters():
        if ("bit" in name) and ("weight" in name):
            if "conv1" in name:
                scale = dataset.num_node_features
            else:
                scale = hidden_units
            w_Byte = scale * par.sum() / 8.0 / 1024.0 + w_Byte
        elif ("bit" in name) and ("fea" in name) and ("gat" not in name):
            a_scale = hidden_units
            a_Byte = a_scale * par.sum() / 8.0 / 1024.0 + a_Byte
        elif ("bit" in name) and ("fea" in name) and ("gat" in name):
            a_scale = hidden_units
            a_Byte = heads * a_scale * par.sum() / 8.0 / 1024.0 + a_Byte
...
class qGATConv(MessagePassing):
    def __init__(
        ...
        super(qGATConv, self).__init__(aggr="add", node_dim=0)
        ...
        if self.is_q:
            self.lin = LinearQuantizationV1(
                in_channels, heads * out_channels, num_nodes, bit=bit, all_positive=all_positive, quant_fea=quant_fea
            )
        else:
            self.lin = nn.Linear(
                in_channels,
        ...
        self.q_gat_attn = WeightQuantizationV1(
            1,
            heads,
            bit=bit,
        )
        self.q_gat_fea = FeatureQuantizationV1(
            input_dim=num_nodes,
            bit=bit,
        )
        self.q_gat_edge = WeightQuantizationV1(
            1,
            heads,
            bit=bit,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--a_loss", type=float, default=0.2)
    ...
    for k in range(1):
        ...
        for i in range(args.max_cycle):
            ...
            print_max_acc = 0
            model = GAT(
                hidden_units, bit, is_q=args.is_q, heads=args.heads, drop_out=args.drop_out, drop_attn=args.drop_attn
            ).to(device)
            ...
            for epoch in range(args.max_epoch):
                ...
                optimizer.zero_grad()
                out = model(data)
                wByte, aByte = parameter_statistics(model, dataset, hidden_units, args.heads)
    ...
    visualization_path = f"{path2result}/{args.model}_{hidden_units}_on_{dataset_name}_{bit}bit-{max_epoch}_is_q{args.is_q}_acc{acc:.3f}_loss{evaluate_loss():.3f}"
