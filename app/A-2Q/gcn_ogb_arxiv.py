...
from torch.nn.functional import dropout, nll_loss, relu
from torch_geometric.nn import GCNConv, MessagePassing
from torch_sparse import SparseTensor, matmul

from a2q.modules import FeatureQuantizationV1, LinearQuantizationV1
from a2q.utils.gcn_norm import gcn_norm
from a2q.utils.logger import Logger
...
def parameter_statistics(model, dataset, hidden_units):
    ...
    for name, par in model.named_parameters():
        ...
        elif ("bit" in name) and (("fea" in name) | ("xw" in name)):
...
class qGCNConv(MessagePassing):
    def __init__(
        ...
        self.q_xw = FeatureQuantizationV1(
            num_nodes, bit, gama_init=para_dict["gama_init"], gama_std=para_dict["gama_std"], is_quantizing_features=False
        )
...
class qGCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, is_q=True, bit=4):
        ...
        if is_q:
            self.convs.append(
                qGCNConv(
                    dataset.num_node_features, hidden_channels, num_nodes, bit, quant_fea=False, all_positive=False
                )
            )
        else:
...
if __name__ == "__main__":
    ...
    dataset = PygNodePropPredDataset(name=args.dataset_name, root=args.path2dataset, transform=T.ToSparseTensor())
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            optimizer.zero_grad()
            out = model(data.x, data.adj_t)[train_idx]
            wByte, aByte = parameter_statistics(model, dataset, args.hidden_channels)
